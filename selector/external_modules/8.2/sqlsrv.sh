...
if [ -e "$PHPEXTDIR/sqlsrv.so" ];then 
	...
	grep "sqlsrv.so" /opt/alt/php-fpm82/usr/php/php.d/sqlsrv.ini 2> /dev/null 1> /dev/null|| echo "extension=sqlsrv.so" > /opt/alt/php-fpm82/usr/php/php.d/sqlsrv.ini
else
...
if [ -e "$PHPEXTDIR/pdo_sqlsrv.so" ];then 
	echo "Creating config file"
	grep "pdo_sqlsrv.so" /opt/alt/php-fpm82/usr/php/php.d/pdo_sqlsrv.ini 2> /dev/null 1> /dev/null|| echo "extension=pdo_sqlsrv.so" > /opt/alt/php-fpm82/usr/php/php.d/pdo_sqlsrv.ini
else
