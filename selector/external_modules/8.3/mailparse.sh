...
if [ -e "$PHPEXTDIR/mailparse.so" ];then 
	...
	grep "mailparse.so" /opt/alt/php-fpm83/usr/php/php.d/mailparse.ini 2> /dev/null 1> /dev/null|| echo "extension=mailparse.so" > /opt/alt/php-fpm83/usr/php/php.d/mailparse.ini
else
