...
if [ -e "$PHPEXTDIR/memcache.so" ];then 
	...
	grep "memcache.so" /opt/alt/php-fpm83/usr/php/php.d/memcache.ini 2> /dev/null 1> /dev/null|| echo "extension=memcache.so" > /opt/alt/php-fpm83/usr/php/php.d/memcache.ini
else
