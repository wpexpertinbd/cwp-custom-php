...
if [ -e "$PHPEXTDIR/mongodb.so" ];then 
	...
	grep "mongodb.so" /opt/alt/php-fpm83/usr/php/php.d/mongodb.ini 2> /dev/null 1> /dev/null|| echo "extension=mongodb.so" > /opt/alt/php-fpm83/usr/php/php.d/mongodb.ini
else
