...
if [ -e "$PHPEXTDIR/xdebug.so" ];then 
	...
	grep "xdebug.so" /opt/alt/php-fpm83/usr/php/php.d/xdebug.ini 2> /dev/null 1> /dev/null|| echo "zend_extension=xdebug.so" > /opt/alt/php-fpm83/usr/php/php.d/xdebug.ini
else
